...
import { useListUnit } from './hook/useListUnit'
...
const EmptyState = () => {
  ...
  return (
    <TableRow>
      <TableCell colSpan={6} sx={{ ...cellSx, py: 8, textAlign: 'center', borderBottom: 0 }}>
        <StraightenOutlined
...
// ── main component ────────────────────────────────────────────────────────
export const ListUnitPage = () => {
  const navigate = useNavigate()
  const theme = useTheme()
  const { t } = useTranslation()
  const isDark = theme.palette.mode === 'dark'
  const { rows, loading, search, setSearch, deleteUnit } = useListUnit()

  const COLUMNS = [
    { id: 'nama', label: t('unit.col_name') },
    { id: 'singkatan', label: t('unit.col_abbr'), width: 110 },
  ...
  const handleEdit = () => {
    navigate(`/satuan/edit/${menuRow.id}`)
    closeMenu()
  }
  const handleDeleteConfirm = async () => {
    await deleteUnit(menuRow.id)
    setDeleteOpen(false)
    setMenuRow(null)
  }
  ...
  const handleExport = () => {
    exportRowsToExcel({
      ...
      sheetName: 'Satuan',
      columns: [
        { header: t('unit.col_name'), key: 'nama' },
        { header: t('unit.col_abbr'), key: 'singkatan' },
  ...
  return (
    <PageLayout
      breadcrumbs={[
        { label: t('unit.breadcrumb_parent'), path: '/produk/list' },
        { label: t('unit.breadcrumb') }
      ]}
            ...
            {loading ? (
              ...
              rows.map((row) => (
                ...
                >
                  ...
                  <TableCell
                    sx={{ ...cellSx, textTransform: 'uppercase', fontSize: 11, fontWeight: 700 }}
                  >
                    {row.singkatan}
                  </TableCell>
      ...
      >
        ...
        <DialogContent>
          <DialogContentText sx={{ fontSize: 13, fontFamily: 'Poppins, sans-serif' }}>
            {t('unit.delete_confirm', { name: menuRow?.nama })}
          </DialogContentText>
        </DialogContent>
